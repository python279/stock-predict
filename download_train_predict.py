...
def download_train_predict(*args):
    ...
    def train_and_predict(code, data_path, predict_path):
        ...
        all_dt = [(datetime.strptime(df.index[0], "%Y-%m-%d") + timedelta(days=i)).__format__('%Y-%m-%d') for i in
                  range(1, (datetime.strptime(df.index[-1], "%Y-%m-%d") - datetime.strptime(df.index[0],
                                                                                            "%Y-%m-%d")).days)]
        miss_data = []
    ...
    return download_code_data(code, start_date, end_date, data_path) and train_and_predict(code, data_path, predict_path)
